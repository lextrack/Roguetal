...
func _on_body_entered(body):
	if body.name == "Player" and body.has_node("PowerUpManager"):
		...
		power_up_manager.activate_double_speed()
		var new_multiplier = power_up_manager.get_speed_multiplier()
		show_speed_increase(new_multiplier)
		queue_free()
...
func show_speed_increase(multiplier: float):
	# Mostrar un mensaje flotante con el nuevo multiplicador de velocidad
	var label = Label.new()
	label.text = "Speed x%.1f!" % multiplier
	label.add_theme_color_override("font_color", Color(0, 1, 0))  # Color verde para velocidad
	add_child(label)
