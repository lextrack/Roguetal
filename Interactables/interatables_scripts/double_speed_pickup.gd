...
func _on_body_entered(body):
	if body.name == "Player" and body.has_node("PowerUpManager"):
		...
		power_up_manager.activate_power_up(PowerUpTypes.PowerUpType.SPEED)
		var new_multiplier = power_up_manager.get_multiplier(PowerUpTypes.PowerUpType.SPEED)
		queue_free()
